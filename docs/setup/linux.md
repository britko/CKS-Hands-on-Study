...
Linux는 Cilium과 Falco를 가장 실제 클러스터에 가깝게 실습할 수 있는 환경입니다. Docker Engine, kind, kubectl, Helm을 설치한 뒤 Bash 스크립트로 클러스터를 생성합니다.
...
Docker Engine은 배포판 공식 절차에 따라 설치합니다. 설치 후 현재 사용자가 Docker socket을 사용할 수 있어야 합니다.
...
Falco의 modern eBPF driver는 커널 기능에 영향을 받습니다. 실패하면 `scripts/install-falco.sh`의 driver 값을 바꾸거나 [Troubleshooting](troubleshooting.md)을 참고합니다.
...
- rootless Docker는 kind 네트워크와 eBPF 실습에서 추가 제약이 있을 수 있습니다.
- cgroup v2와 커널 버전에 따라 Cilium kube-proxy replacement나 Falco driver 동작이 달라질 수 있습니다.
- 이 저장소는 안정성을 위해 kube-proxy replacement를 기본으로 사용하지 않습니다.

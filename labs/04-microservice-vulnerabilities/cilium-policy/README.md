...
L7 proxy를 켜기 어려운 환경에서는 이 랩을 CiliumNetworkPolicy YAML 구조 학습으로 진행하고, 표준 NetworkPolicy 실습은 [NetworkPolicy](../../01-cluster-setup/network-policy/README.md)에서 완료합니다.
...
```bash
kubectl apply -f labs/04-microservice-vulnerabilities/cilium-policy/manifests/app.yaml
kubectl apply -f labs/04-microservice-vulnerabilities/cilium-policy/manifests/cilium-network-policy.yaml
```
...
```bash
kubectl delete -f labs/04-microservice-vulnerabilities/cilium-policy/manifests/cilium-network-policy.yaml
kubectl delete -f labs/04-microservice-vulnerabilities/cilium-policy/manifests/app.yaml
```

...
```powershell
Get-Content labs/07-audit-logging/manifests/audit-policy.yaml
```

API 요청을 만들 리소스 적용:

```powershell
kubectl apply -f labs/07-audit-logging/manifests/audit-target.yaml
...
```powershell
kubectl delete -f labs/07-audit-logging/manifests/audit-target.yaml
```
